/* A network driver using virtio with co-virtual switch functions.
 *
 * Copyright 2007 Rusty Russell <rusty@rustcorp.com.au> IBM Corporation
 * Copyright 2013-2014 Ryota Kawashima <kawa1983@nitech.ac.jp> Nagoya Institute of Technology
 *
...
static inline bool cvsw_handle_tx_skb(struct sk_buff *skb)
{
    return cvsw_handle_data(skb, CVSW_PORT_HOST);
}

static inline bool cvsw_handle_rx_skb(struct sk_buff *skb)
{
    if (unlikely(ntohs(skb->protocol) == CVSW_ETH_TYPE)) {
	cvsw_handle_ctl(skb);
	return false; /* Drop the CVSW message */
    } else if (likely(cvsw_state == CVSW_STATE_CONNECTED)) {
	bool ret;
	skb_push(skb, sizeof(struct ethhdr));
	ret = cvsw_handle_data(skb, CVSW_PORT_NET);
 ...
 ***************************************************************************/
	...
	if (likely(cvsw && skb->protocol != htons(CVSW_ETH_TYPE))) {
	    if (cvsw_state == CVSW_STATE_CONNECTED) {
		if (unlikely(! cvsw_handle_tx_skb(skb))) {
...
static int virtnet_probe(struct virtio_device *vdev)
{
	...
	if (err) {
		pr_debug("cvsw_net: registering device failed\n");
		goto free_vqs;
	}
	...
	if (err) {
		pr_debug("cvsw_net: registering cpu notifier failed\n");
		goto free_recv_bufs;
	}
...
{
    return (start_xmit(skb, skb->dev) == NETDEV_TX_OK);
}

extern bool cvsw_change_mtu(struct net_device *dev, const int mtu)
{
    pr_info("Change MTU size from %d to %d\n", dev->mtu, mtu);

    return (virtnet_change_mtu(dev, mtu) == 0);
}

extern void cvsw_change_offload(struct net_device *dev, const __u16 offload)
{
    if (offload & CVSW_OFFLOAD_CSUM) {
	dev->features |= NETIF_F_HW_CSUM;
    } else {
	dev->features ^= NETIF_F_HW_CSUM;
    }
    if (offload & CVSW_OFFLOAD_TSO) {
	dev->features |= NETIF_F_ALL_TSO;
    } else {
	dev->features ^= NETIF_F_ALL_TSO;
    }
    if (offload & CVSW_OFFLOAD_UFO) {
	dev->features |= NETIF_F_UFO;
    } else {
	dev->features ^= NETIF_F_UFO;
    }
    if (offload & CVSW_OFFLOAD_GSO) {
	dev->features |= NETIF_F_GSO;
    } else {
	dev->features ^= NETIF_F_GSO;
    }
    if (offload & CVSW_OFFLOAD_GRO) {
...
MODULE_DESCRIPTION("Virtio network driver with co-vswitch functions");
MODULE_LICENSE("GPL");
